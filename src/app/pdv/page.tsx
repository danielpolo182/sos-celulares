...
const FORMAS: { key: FormaPagKey; label: string; icon: string; cor: string }[] = [
  { key: 'dinheiro',        label: 'Dinheiro',    icon: 'ðŸ’µ', cor: '#16a34a' },
  { key: 'pix',             label: 'PIX',         icon: 'ðŸ“±', cor: '#2563eb' },
  { key: 'debito',          label: 'DÃ©bito',      icon: 'ðŸ’³', cor: '#8b5cf6' },
  { key: 'credito_avista',  label: 'CrÃ©d. Ã  vista',icon: 'ðŸ’³', cor: '#0ea5e9' },
  { key: 'credito_parcela', label: 'CrÃ©d. Parc.', icon: 'ðŸ’³', cor: '#f59e0b' },
  { key: 'transferencia',   label: 'Transfer.',   icon: 'ðŸ¦', cor: '#64748b' },
]

const CATEGORIA_ICONES: Record<string, string> = {
  'Display / Tela': 'ðŸ–¥', 'Bateria': 'ðŸ”‹', 'Conector de carga': 'âš¡',
  'CÃ¢mera': 'ðŸ“·', 'Alto-falante': 'ðŸ”Š', 'Outros': 'ðŸ“¦',
}
...
export default function PDVPage() {
  ...
  const fetchProdutos = useCallback(async () => {
    ...
    // Buscar mais vendidos dos Ãºltimos 30 dias
  ...
  function calcularTotalComTaxa(valor: number, forma: FormaPagKey, parcelas: number): { total: number; taxa: number; taxaEfetivaPct: number } {
    ...
    // Usa taxa especÃ­fica da parcela se disponÃ­vel; senÃ£o usa taxa_pct geral
    ...
    // Taxa efetiva = acrÃ©scimo real sobre o valor original
    const taxaEfetivaPct = valor > 0 ? +((taxa / valor) * 100).toFixed(2) : 0
    return { total, taxa, taxaEfetivaPct }
  }

  function adicionarPagamento() {
    // total jÃ¡ inclui a taxa reativamente â€” apenas registra o valor que serÃ¡ pago
  ...
  async function salvarCaixa() {
    ...
    if (caixaAcao === 'reabertura') {
      // Remove abertura e fechamento do dia para comeÃ§ar perÃ­odo limpo
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, fontFamily: 'var(--font-sans)', background: '#f8fafc', overflow: 'hidden', minHeight: 0 }}>
      <style>{`
        ...
        @media (max-width: 767px) {
          ...
          .pdv-mob-tab.active { color: #2563eb; border-bottom-color: #2563eb; background: #eff6ff; }
          ...
          .pdv-cart-badge { display: inline-block; background: #2563eb; color: #fff; border-radius: 10px; font-size: 10px; padding: 1px 5px; margin-left: 4px; }
        }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: '#fff', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
        <div style={{ display: 'flex', gap: 4 }}>
          {([['venda', 'ðŸ›’ Venda'], ['caixa', 'ðŸ’° Caixa'], ['historico', 'ðŸ“‹ HistÃ³rico']] as const).map(([k, l]) => (
            <button key={k} onClick={() => setAba(k)} style={{ padding: '8px 16px', fontSize: 13, fontWeight: aba === k ? 600 : 400, border: 'none', background: aba === k ? '#dbeafe' : 'transparent', cursor: 'pointer', color: aba === k ? '#2563eb' : '#64748b', borderRadius: 7 }}>{l}</button>
          ))}
        </div>
        {caixaAberta && <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 500, padding: '3px 10px', borderRadius: 20, background: '#ecfdf5', color: '#065f46' }}>âœ… Caixa aberto Â· {formatMoeda(caixaHoje?.saldo_atual ?? 0)}</span>}
        {!caixaAberta && <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 500, padding: '3px 10px', borderRadius: 20, background: '#fef2f2', color: '#991b1b' }}>ðŸ”’ Caixa {caixaFechada ? 'fechado' : 'nÃ£o aberto'}</span>}
      </div>

      {/* Mobile tabs (sÃ³ aparece na aba venda em telas < 768px) */}
      {aba === 'venda' && (
        <div className="pdv-mob-tabs">
          <button className={`pdv-mob-tab${mobileView === 'produtos' ? ' active' : ''}`} onClick={() => setMobileView('produtos')}>ðŸ› Produtos</button>
          <button className={`pdv-mob-tab${mobileView === 'carrinho' ? ' active' : ''}`} onClick={() => setMobileView('carrinho')}>
            ðŸ›’ Carrinho{itens.length > 0 && <span className="pdv-cart-badge">{itens.length}</span>}
          </button>
        </div>
      )}
      ...
      <div className="pdv-cols">
        ...
        {/* â”€â”€ ABA VENDA â€” layout dois colunas */}
        {aba === 'venda' && (
          <>
            ...
            <div className={`pdv-left${mobileView === 'produtos' ? ' mob-active' : ''}`}>
              ...
              <div style={{ padding: '12px 16px', background: '#fff', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
                <div style={{ position: 'relative' }}>
                  <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 16, color: '#94a3b8', pointerEvents: 'none' }}>ðŸ”</span>
                  <input
                      ...
                      {prodResults.map(p => (
                        <div key={p.id} onClick={() => adicionarProduto(p)} style={{ padding: '10px 14px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', fontSize: 13 }}
                          ...
                          <div>
                            ...
                            <p style={{ fontSize: 11, color: '#94a3b8' }}>Estoque: {p.estoque_atual} Â· {p.categoria ?? 'Sem categoria'}</p>
                          </div>
                          <p style={{ fontWeight: 700, color: '#2563eb', whiteSpace: 'nowrap', marginLeft: 12 }}>{formatMoeda(p.preco_venda)}</p>
                        </div>
                      ))}
                        ...
                        <div>
                          ...
                          {!showQuickAdd ? (
                            <div onClick={() => setShowQuickAdd(true)}
                              ...
                              âž• Cadastrar &quot;{searchProd}&quot; como novo produto
                            </div>
                          ) : (
                            <div style={{ padding: '10px 14px', background: '#f0fdf4', borderTop: '1px solid #dcfce7' }}>
                              ...
                              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                <div style={{ flex: 1 }}>
                                  <label style={{ ...lbl, marginBottom: 2 }}>PreÃ§o de venda (R$) *</label>
                                  <input
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
                {!searchProd && (
                  <>
                    <p style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>â­ Mais vendidos (30 dias)</p>
                      ...
                      {maisVendidos.map(p => {
                        ...
                        return (
                          <button key={p.id} onClick={() => !semEstoque && adicionarProduto(p)} disabled={semEstoque}
                            style={{ padding: '14px 12px', borderRadius: 12, border: `2px solid ${qtdNoCarrinho > 0 ? '#60a5fa' : '#e2e8f0'}`, background: qtdNoCarrinho > 0 ? '#dbeafe' : semEstoque ? '#f8fafc' : '#fff', cursor: semEstoque ? 'not-allowed' : 'pointer', textAlign: 'center', opacity: semEstoque ? 0.5 : 1, transition: 'all 0.12s' }}>
                            <div style={{ fontSize: 28, marginBottom: 6 }}>{CATEGORIA_ICONES[p.categoria ?? ''] ?? 'ðŸ“¦'}</div>
                            <div style={{ fontSize: 12, fontWeight: 600, color: qtdNoCarrinho > 0 ? '#1d4ed8' : '#0f172a', marginBottom: 4, lineHeight: 1.3 }}>{p.nome.length > 28 ? p.nome.slice(0, 26) + 'â€¦' : p.nome}</div>
                            <div style={{ fontSize: 15, fontWeight: 800, color: qtdNoCarrinho > 0 ? '#2563eb' : '#2563eb' }}>{formatMoeda(p.preco_venda)}</div>
                            {qtdNoCarrinho > 0 && <div style={{ fontSize: 10, color: '#2563eb', marginTop: 3, fontWeight: 600 }}>Ã— {qtdNoCarrinho} no carrinho</div>}
            ...
            <div className={`pdv-right${mobileView === 'carrinho' ? ' mob-active' : ''}`}>
              ...
              {vendaOk && ultimaVenda && (
                <div style={{ padding: '14px 16px', background: '#ecfdf5', borderBottom: '1px solid #bbf7d0', flexShrink: 0 }}>
                  <p style={{ fontSize: 14, fontWeight: 600, color: '#065f46', marginBottom: 6 }}>âœ… Venda #{ultimaVenda.numero} finalizada!</p>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={imprimir} style={{ flex: 1, padding: '7px', border: '1px solid #86efac', borderRadius: 7, fontSize: 12, cursor: 'pointer', background: '#fff', color: '#065f46', fontWeight: 500 }}>ðŸ–¨ Imprimir</button>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
                ...
                {itens.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '40px 10px', color: '#94a3b8' }}><div style={{ fontSize: 32, marginBottom: 8 }}>ðŸ›’</div><p style={{ fontSize: 13 }}>Selecione produtos ao lado</p></div>
                ) : itens.map((item, i) => (
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        {item.produto_id
                          ...
                          : <input style={{ ...inp, fontSize: 12, marginBottom: 5, padding: '5px 8px' }} value={item.descricao} onChange={e => atualizarItem(i, 'descricao', e.target.value)} placeholder="DescriÃ§Ã£o..." />}
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          <button onClick={() => atualizarItem(i, 'quantidade', Math.max(1, item.quantidade - 1))} style={{ width: 26, height: 26, borderRadius: '50%', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>âˆ’</button>
                          ...
                          <span style={{ color: '#94a3b8', fontSize: 11, marginLeft: 2 }}>Ã—</span>
                      ...
                      <button onClick={() => removerItem(i)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: 18, padding: 0, flexShrink: 0, lineHeight: 1 }}>Ã—</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="pdv-right-bottom">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, color: '#64748b', marginBottom: 5 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    {desconto > 0 && <span style={{ color: '#065f46', fontWeight: 500, fontSize: 11 }}>âˆ’ {formatMoeda(desconto)}</span>}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 18, fontWeight: 700, color: '#2563eb', paddingTop: 8, borderTop: '1px solid #e2e8f0', marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: 12, color: '#92400e' }}>
                        ...
                        {taxaFormaAtual > 0 && (
                          <span style={{ marginLeft: 8, fontSize: 11, color: '#b45309' }}>
                            (acrÃ©scimo real: <strong>{taxaEfetivaAtual}%</strong> = +{formatMoeda(taxaFormaAtual)})
                          </span>
                        )}
                      </div>
                    )}
                ...
                <div style={{ display: 'flex', gap: 7, marginBottom: 8 }}>
                  ...
                  <button onClick={adicionarPagamento} style={{ padding: '7px 14px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>+</button>
                </div>
                {pagamentos.map((p, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 9px', background: '#eff6ff', border: '1px solid #dbeafe', borderRadius: 6, marginBottom: 5, fontSize: 12 }}>
                    <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <button onClick={() => setPagamentos(prev => prev.filter((_, idx) => idx !== i))} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: 15, lineHeight: 1 }}>Ã—</button>
                    </div>
                  </div>
                ))}
                ...
                <div style={{ marginBottom: 8 }}>
                  <input style={{ ...inp, fontSize: 12, padding: '7px 10px' }} value={obs} onChange={e => setObs(e.target.value)} placeholder="ObservaÃ§Ãµes (opcional)..." />
                </div>
                ...
                <button onClick={finalizarVenda} disabled={salvando || itens.length === 0 || faltaPagar > 0.01}
                  ...
                  {salvando ? 'Finalizando...' : itens.length === 0 ? 'Adicione itens' : faltaPagar > 0.01 ? `Falta ${formatMoeda(faltaPagar)}` : `âœ“ Finalizar Â· ${formatMoeda(total)}`}
                </button>
              </div>
            </div>
          </>
            ...
            <div style={{ maxWidth: 620 }}>
              ...
              <div style={{ background: caixaAberta ? '#f0fdf4' : '#fef2f2', border: `1px solid ${caixaAberta ? '#bbf7d0' : '#fecaca'}`, borderRadius: 12, padding: '16px 20px', marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 600, color: caixaAberta ? '#065f46' : '#991b1b' }}>{caixaAberta ? 'âœ… Caixa aberto' : caixaFechada ? 'ðŸ”’ Caixa fechado hoje' : 'âš ï¸ Caixa nÃ£o foi aberto hoje'}</p>
                    {caixaHoje?.aberto_em && <p style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>Aberto Ã s {new Date(caixaHoje.aberto_em).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}{caixaHoje.fechado_em ? ` Â· Fechado Ã s ${new Date(caixaHoje.fechado_em).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : ''}</p>}
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontSize: 24, fontWeight: 700, color: '#0f172a' }}>{formatMoeda(caixaHoje?.saldo_atual ?? 0)}</p>
                    <p style={{ fontSize: 11, color: '#64748b' }}>Saldo atual Â· {caixaHoje?.total_vendas ?? 0} venda(s)</p>
                  </div>
                </div>
              </div>

              {/* Cards entradas/saÃ­das */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10, marginBottom: 16 }}>
                {[
                  { label: 'Entradas', value: formatMoeda(caixaHoje?.entradas ?? 0), color: '#065f46', bg: '#ecfdf5' },
                  { label: 'SaÃ­das', value: formatMoeda(caixaHoje?.saidas ?? 0), color: '#991b1b', bg: '#fef2f2' },
                ].map(m => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8, marginBottom: 20 }}>
                {[
                  { acao: 'abertura' as const,   label: 'ðŸ”“ Abrir caixa',    disabled: caixaAberta || caixaFechada },
                  { acao: 'fechamento' as const,  label: 'ðŸ”’ Fechar caixa',   disabled: !caixaAberta },
                  { acao: 'sangria' as const,     label: 'ðŸ’¸ Sangria',         disabled: !caixaAberta },
                  { acao: 'suprimento' as const,  label: 'ðŸ’µ Suprimento',      disabled: !caixaAberta },
                ].map(b => (
              ...
              {/* BotÃ£o reabrir (aparece sÃ³ quando caixa foi fechado) */}
              {caixaFechada && (
                <button onClick={() => { setCaixaAcao('reabertura'); setCaixaValor(''); setCaixaObs(''); setShowCaixaModal(true) }}
                  ...
                  ðŸ”„ Reabrir caixa (novo perÃ­odo)
                </button>
              )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#fff', borderRadius: 14, width: 360, padding: '24px' }}>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: '#0f172a', marginBottom: 8 }}>
              {caixaAcao === 'abertura' ? 'ðŸ”“ Abrir caixa' : caixaAcao === 'fechamento' ? 'ðŸ”’ Fechar caixa' : caixaAcao === 'reabertura' ? 'ðŸ”„ Reabrir caixa' : caixaAcao === 'sangria' ? 'ðŸ’¸ Sangria' : 'ðŸ’µ Suprimento'}
            </h3>
            {(caixaAcao === 'abertura' || caixaAcao === 'reabertura') && (
              <p style={{ fontSize: 12, color: '#64748b', marginBottom: 14 }}>Informe o saldo fÃ­sico contado na gaveta no momento da abertura.</p>
            )}
            {caixaAcao === 'fechamento' && (
              <p style={{ fontSize: 12, color: '#64748b', marginBottom: 14 }}>Informe o saldo fÃ­sico contado na gaveta agora.</p>
            )}
            ...
            <div style={{ marginBottom: 16 }}><label style={lbl}>ObservaÃ§Ãµes</label><input style={inp} value={caixaObs} onChange={e => setCaixaObs(e.target.value)} placeholder="Opcional..." /></div>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={salvarCaixa} disabled={salvandoCaixa || !caixaValor} style={{ flex: 2, padding: '9px', background: !caixaValor ? '#e2e8f0' : '#2563eb', color: !caixaValor ? '#94a3b8' : '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: !caixaValor ? 'not-allowed' : 'pointer' }}>
...
function HistoricoCaixa({ supabase }: { supabase: ReturnType<typeof createClient> }) {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: '#94a3b8', padding: 40 }}>Carregando histÃ³rico...</div>
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      <p style={{ fontSize: 12, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 14 }}>ðŸ“‹ HistÃ³rico de Caixa</p>
      {periodos.map(p => {
          ...
          <div key={p.data} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '14px 18px', marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <div>
                ...
                <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>
                  {p.abertura ? `Abertura: ${new Date(p.abertura.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : 'NÃ£o aberto'}
                  {p.fechamento ? ` Â· Fechamento: ${new Date(p.fechamento.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : ' Â· Ainda aberto'}
                </p>
              </div>
              <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: p.fechamento ? '#f1f5f9' : '#ecfdf5', color: p.fechamento ? '#475569' : '#065f46' }}>
                {p.fechamento ? 'Fechado' : 'ðŸŸ¢ Aberto'}
              </span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
              {[
                ...
                { label: 'Fechamento', value: fechamento !== null ? formatMoeda(fechamento) : 'â€”', color: '#374151' },
                { label: 'DiferenÃ§a', value: diferenca !== null ? formatMoeda(diferenca) : 'â€”', color: diferenca === null ? '#94a3b8' : diferenca > 0 ? '#065f46' : diferenca < 0 ? '#991b1b' : '#374151' },
              ].map(c => (
            ...
            {(p.sangrias > 0 || p.suprimentos > 0) && (
              <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 11, color: '#64748b' }}>
                {p.sangrias > 0 && <span>ðŸ’¸ Sangria: {formatMoeda(p.sangrias)}</span>}
                {p.suprimentos > 0 && <span>ðŸ’µ Suprimento: {formatMoeda(p.suprimentos)}</span>}
              </div>
            )}
          </div>
        )
