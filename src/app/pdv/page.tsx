...
type Pagamento = {
  forma: 'dinheiro' | 'pix' | 'credito' | 'debito'
  valor: number
}
...
const FORMAS: { key: 'dinheiro' | 'pix' | 'credito' | 'debito'; label: string; icon: string }[] = [
...
export default function PDVPage() {
  ...
  useEffect(() => { fetchProdutos(); fetchCaixa() }, [fetchProdutos, fetchCaixa])
  ...
  function buscarProduto(q: string) {
    ...
    searchTimer.current = setTimeout(() => {
      ...
      const r = produtos.filter(p => p.nome.toLowerCase().includes(q.toLowerCase())).slice(0, 8)
      setProdResults(r)
    }, 200)
  }
  ...
  function adicionarProduto(p: Produto) {
    ...
    if (exist >= 0) {
      const novo = [...itens]
      novo[exist].quantidade++
      novo[exist].subtotal = novo[exist].quantidade * novo[exist].preco_unit
      setItens(novo)
    } else {
      setItens(prev => [...prev, {
        produto_id: p.id, descricao: p.nome,
        quantidade: 1, preco_unit: p.preco_venda,
        custo_unit: p.custo_medio, subtotal: p.preco_venda,
      }])
    }
  ...
  function removerPagamento(i: number) { setPagamentos(prev => prev.filter((_, idx) => idx !== i)) }
  ...
  async function finalizarVenda() {
    ...
    if (venda) {
      ...
      // Baixar estoque dos produtos vinculados
      ...
      // Emitir evento
      await supabase.from('events').insert({ type: 'VENDA_REALIZADA', entity: 'venda', entity_id: venda.id, payload: { total, itens: itens.length } })
  ...
  async function salvarCaixa() {
    ...
    await supabase.from('caixa_movimentos').insert({
      tipo: caixaAcao, valor: v,
      forma: 'dinheiro', observacoes: caixaObs || null,
    })
  ...
  function imprimir() {
    ...
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:monospace;font-size:12px;max-width:300px;margin:0 auto;padding:16px}
    .center{text-align:center}.bold{font-weight:bold}.line{border-top:1px dashed #000;margin:8px 0}
    .row{display:flex;justify-content:space-between;margin:3px 0}</style></head><body>
    <div class="center bold" style="font-size:16px">SOS Celulares</div>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'var(--font-sans)', overflow: 'hidden', background: '#f8fafc' }}>

      {/* PAINEL ESQUERDO — produtos e itens */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid #e2e8f0' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', background: '#fff' }}>
          {([['venda', '🛒 Venda'], ['caixa', '💰 Caixa'], ['historico', '📋 Histórico']] as const).map(([k, l]) => (
            <button key={k} onClick={() => setAba(k)} style={{
              padding: '12px 20px', fontSize: 13, fontWeight: aba === k ? 600 : 400,
        ...
        {aba === 'venda' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 16px', borderBottom: '1px solid #f1f5f9', background: '#fff' }}>
              <div style={{ position: 'relative' }}>
                ...
                {prodResults.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.1)', marginTop: 4, overflow: 'hidden' }}>
                    {prodResults.map(p => (
                      <div key={p.id} onClick={() => adicionarProduto(p)} style={{
                        ...
                        <div>
                          ...
                          <p style={{ fontSize: 11, color: '#94a3b8' }}>{p.categoria} · Estoque: {p.estoque_atual}</p>
                        </div>
                        <p style={{ fontWeight: 600, color: '#6366f1' }}>{formatMoeda(p.preco_venda)}</p>
                      </div>
                    ))}
                    <div onClick={adicionarAvulso} style={{ padding: '10px 14px', cursor: 'pointer', fontSize: 13, color: '#6366f1', fontWeight: 500, background: '#f8f7ff' }}
              ...
              <button onClick={adicionarAvulso} style={{ marginTop: 8, fontSize: 12, color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
              {itens.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 20px', color: '#94a3b8' }}>
                  ...
                  <p style={{ fontSize: 14 }}>Busque um produto para começar a venda</p>
                </div>
              ) : itens.map((item, i) => (
                <div key={i} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      {item.produto_id ? (
                        <p style={{ fontSize: 13, fontWeight: 500, color: '#0f172a', marginBottom: 6 }}>{item.descricao}</p>
                      ) : (
                        <input style={{ ...inp, marginBottom: 6 }} value={item.descricao} onChange={e => atualizarItem(i, 'descricao', e.target.value)} placeholder="Descrição do item..." />
                      )}
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <div style={{ flex: 2 }}>
                          <label style={lbl}>Preço unit (R$)</label>
                          <input style={inp} type="number" step="0.01" value={item.preco_unit} onChange={e => atualizarItem(i, 'preco_unit', parseFloat(e.target.value) || 0)} />
                        </div>
                        <div style={{ flex: 2, paddingTop: 16 }}>
                          <p style={{ fontSize: 14, fontWeight: 600, color: '#6366f1' }}>{formatMoeda(item.subtotal)}</p>
                        </div>
                      </div>
                    </div>
        ...
        {aba === 'caixa' && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
            {/* Status do caixa */}
            <div style={{
              background: caixaAberta ? '#f0fdf4' : '#fef2f2',
              border: `1px solid ${caixaAberta ? '#bbf7d0' : '#fecaca'}`,
              borderRadius: 12, padding: '16px 20px', marginBottom: 16,
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600, color: caixaAberta ? '#065f46' : '#991b1b' }}>
                    {caixaAberta ? '✅ Caixa aberto' : '🔒 Caixa fechado'}
                  </p>
                  {caixaHoje?.aberto_em && <p style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>
                    Aberto às {new Date(caixaHoje.aberto_em).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                  </p>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{formatMoeda(caixaHoje?.saldo_atual ?? 0)}</p>
                  <p style={{ fontSize: 11, color: '#64748b' }}>Saldo atual</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
              {[
                ...
                { label: 'Vendas', value: String(caixaHoje?.total_vendas ?? 0), color: '#3730a3', bg: '#eef2ff' },
              ].map(m => (
                <div key={m.label} style={{ background: m.bg, borderRadius: 10, padding: '12px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
              {[
                ...
                <button key={b.acao} disabled={b.disabled} onClick={() => { setCaixaAcao(b.acao); setCaixaValor(''); setCaixaObs(''); setShowCaixaModal(true) }} style={{
                  padding: '10px', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 13, fontWeight: 500,
                  cursor: b.disabled ? 'not-allowed' : 'pointer',
                  background: b.disabled ? '#f8fafc' : '#fff',
                  color: b.disabled ? '#94a3b8' : '#374151',
            ...
            {movimentos.length === 0 ? (
              <p style={{ fontSize: 13, color: '#94a3b8', textAlign: 'center', padding: '20px 0' }}>Nenhum movimento registrado</p>
            ) : movimentos.map(m => (
              <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#fff', border: '1px solid #f1f5f9', borderRadius: 8, marginBottom: 6, fontSize: 13 }}>
                <div>
                  <span style={{ fontWeight: 500, color: '#374151', textTransform: 'capitalize' }}>{m.tipo}</span>
                  {m.observacoes && <span style={{ color: '#94a3b8', marginLeft: 8 }}>{m.observacoes}</span>}
                </div>
      ...
      <div style={{ width: 320, display: 'flex', flexDirection: 'column', background: '#fff', overflow: 'hidden' }}>
        ...
        {/* Sucesso */}
        {vendaOk && ultimaVenda && (
          <div style={{ padding: '16px', background: '#ecfdf5', borderBottom: '1px solid #bbf7d0' }}>
            <p style={{ fontSize: 14, fontWeight: 600, color: '#065f46', marginBottom: 4 }}>✅ Venda #{ultimaVenda.numero} finalizada!</p>
            {ultimaVenda.troco > 0 && <p style={{ fontSize: 13, color: '#065f46' }}>Troco: {formatMoeda(ultimaVenda.troco)}</p>}
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <button onClick={imprimir} style={{ flex: 1, padding: '7px', border: '1px solid #86efac', borderRadius: 7, fontSize: 12, cursor: 'pointer', background: '#fff', color: '#065f46', fontWeight: 500 }}>🖨 Imprimir recibo</button>
        ...
        <div style={{ padding: '16px', borderBottom: '1px solid #f1f5f9' }}>
          <p style={{ fontSize: 12, fontWeight: 500, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 10 }}>Resumo</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 18, fontWeight: 700, color: '#0f172a', marginTop: 10, paddingTop: 10, borderTop: '1px solid #f1f5f9' }}>
            <span>Total</span>
            <span style={{ color: '#6366f1' }}>{formatMoeda(total)}</span>
          </div>
        </div>

        {/* Pagamentos */}
        <div style={{ padding: '16px', borderBottom: '1px solid #f1f5f9', flex: 1, overflowY: 'auto' }}>
          <p style={{ fontSize: 12, fontWeight: 500, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 10 }}>Pagamento</p>

          {/* Formas */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, marginBottom: 10 }}>
            {FORMAS.map(f => (
              <button key={f.key} onClick={() => setFormaPag(f.key)} style={{
                padding: '8px 4px', borderRadius: 8, border: '1px solid', cursor: 'pointer', fontSize: 11, fontWeight: 500,
                textAlign: 'center', flexDirection: 'column' as const,
                background: formaPag === f.key ? '#e0e7ff' : '#f8fafc',
                color: formaPag === f.key ? '#3730a3' : '#64748b',
                borderColor: formaPag === f.key ? '#818cf8' : '#e2e8f0',
              }}>
                <div>{f.icon}</div>
                <div>{f.label}</div>
              </button>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            <input
              type="number" step="0.01" value={valorPag}
          ...
          {pagamentos.map((p, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', background: '#f8f7ff', border: '1px solid #e0e7ff', borderRadius: 7, marginBottom: 6, fontSize: 13 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={() => removerPagamento(i)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>×</button>
              </div>
            </div>
          ))}
          ...
          {/* Totais pagamento */}
          {pagamentos.length > 0 && (
            <div style={{ marginTop: 10, padding: '10px', background: '#f8fafc', borderRadius: 8, fontSize: 13 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ color: '#64748b' }}>Total pago</span>
                <span style={{ fontWeight: 500 }}>{formatMoeda(totalPago)}</span>
              </div>
              {faltaPagar > 0.01 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#991b1b' }}>
                  <span>Falta pagar</span>
                  <span style={{ fontWeight: 600 }}>{formatMoeda(faltaPagar)}</span>
                </div>
              )}
              {troco > 0.01 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#065f46', fontWeight: 600, fontSize: 15 }}>
                  <span>Troco</span>
                  <span>{formatMoeda(troco)}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ padding: '16px' }}>
          <button
            ...
            disabled={salvando || itens.length === 0 || faltaPagar > 0.01}
            style={{
              width: '100%', padding: '14px',
              background: salvando || itens.length === 0 || faltaPagar > 0.01 ? '#e2e8f0' : '#6366f1',
              color: salvando || itens.length === 0 || faltaPagar > 0.01 ? '#94a3b8' : '#fff',
              border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 600,
              cursor: salvando || itens.length === 0 || faltaPagar > 0.01 ? 'not-allowed' : 'pointer',
            }}
          >
            {salvando ? 'Finalizando...' : itens.length === 0 ? 'Adicione itens' : faltaPagar > 0.01 ? `Falta ${formatMoeda(faltaPagar)}` : `✓ Finalizar venda · ${formatMoeda(total)}`}
          </button>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#fff', borderRadius: 14, width: 360, padding: '24px' }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <input style={inp} type="number" step="0.01" value={caixaValor} onChange={e => setCaixaValor(e.target.value)} placeholder="0,00" autoFocus />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={lbl}>Observações</label>
              <input style={inp} value={caixaObs} onChange={e => setCaixaObs(e.target.value)} placeholder="Opcional..." />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
...
function HistoricoVendas({ supabase }: { supabase: ReturnType<typeof createClient> }) {
  ...
  return (
    <div>
      {vendas.map(v => (
        <div key={v.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: '#6366f1' }}>Venda #{v.numero}</p>
              <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>{new Date(v.created_at).toLocaleString('pt-BR')}</p>
            </div>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#0f172a' }}>R$ {Number(v.total).toFixed(2).replace('.', ',')}</p>
          </div>
