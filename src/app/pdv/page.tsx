                      ...
                      {maisVendidos.map(p => {
                        ...
                        return (
                          <button key={p.id} onClick={() => !semEstoque && adicionarProduto(p)} disabled={semEstoque}
                            ...
                            <div style={{ fontSize: 12, fontWeight: 600, color: qtdNoCarrinho > 0 ? '#1d4ed8' : '#0f172a', marginBottom: 4, lineHeight: 1.3 }}>{p.nome.length > 28 ? p.nome.slice(0, 26) + '…' : p.nome}</div>
                            <div style={{ fontSize: 15, fontWeight: 800, color: qtdNoCarrinho > 0 ? '#2563eb' : '#2563eb' }}>{formatMoeda(p.preco_venda)}</div>
                            {qtdNoCarrinho > 0 && <div style={{ fontSize: 10, color: '#2563eb', marginTop: 3, fontWeight: 600 }}>× {qtdNoCarrinho} no carrinho</div>}
