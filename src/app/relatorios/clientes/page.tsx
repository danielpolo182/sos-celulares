...
export default function RelClientes() {
  ...
  return (
        ...
        <div style={{ background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,overflow:'hidden' }}>
          <table style={{ width:'100%',borderCollapse:'collapse',fontSize:13 }}>
            ...
            <tbody>{dados.map((c:any)=>(
              <tr key={c.id} onClick={() => setClienteSelecionado(c.id)} style={{ borderBottom:'1px solid #f1f5f9', cursor:'pointer', transition:'background 0.15s' }} onMouseEnter={e=>(e.currentTarget.style.background='#f8fafc')} onMouseLeave={e=>(e.currentTarget.style.background='')}>
    ...
    {clienteSelecionado && <ClienteDetalheModal clienteId={clienteSelecionado} onClose={() => setClienteSelecionado(null)} />}
    </>
  )
}
