...
export default function EstoquePage() {
  ...
  // â”€â”€ Form compartilhado (rÃ¡pido + completo)
  ...
  // â”€â”€ Campos extras (sÃ³ modal completo)
  ...
  // Abre modal rÃ¡pido automaticamente se vier com ?novo=<nome> na URL
  useEffect(() => {
  ...
  // â”€â”€ Linha da tabela de produtos
    ...
    return (
      <tr style={{ borderBottom: '1px solid #f1f5f9', background: showDivider ? '#fffbeb' : undefined }}
        ...
        <td style={{ padding: '10px 14px', color: '#374151', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
          {custo > 0 ? `R$ ${custo.toFixed(2).replace('.', ',')}` : <span style={{ color: '#fbbf24' }}>â€”</span>}
        </td>
        <td style={{ padding: '10px 14px', fontWeight: 500, color: preco > 0 ? '#0f172a' : '#fbbf24', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
          {preco > 0 ? `R$ ${preco.toFixed(2).replace('.', ',')}` : 'â€”'}
        </td>
        <td style={{ padding: '10px 14px' }}>
          {margem ? <span style={{ fontSize: 12, fontWeight: 600, color: parseInt(margem) >= 30 ? '#065f46' : parseInt(margem) >= 15 ? '#92400e' : '#991b1b' }}>{margem}%</span> : 'â€”'}
        </td>
        <td style={{ padding: '10px 14px' }}>
        ...
        <td style={{ padding: '10px 14px' }}>
          <div style={{ display: 'flex', gap: 5 }}>
            ...
            <button onClick={e => { e.stopPropagation(); abrirRapido(p) }} style={{ fontSize: 11, padding: '4px 8px', border: '1px solid #e2e8f0', borderRadius: 6, background: '#fff', color: '#64748b', cursor: 'pointer', whiteSpace: 'nowrap' }}>Editar rÃ¡pido</button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'var(--font-sans)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 24px 0', borderBottom: '1px solid #e2e8f0', background: '#fff', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>
              {produtos.length} produtos Â· {incompletos.length > 0 ? <span style={{ color: '#d97706' }}>{incompletos.length} incompletos</span> : 'todos completos'}
            </p>
          </div>
          {aba === 'produtos' && (
        ...
        <div style={{ display: 'flex', gap: 2 }}>
          {([['produtos','ðŸ“¦ Produtos & Estoque'],['entradas','ðŸ“¥ Entradas'],['fornecedores','ðŸ­ Fornecedores'],['pedidos','ðŸ”§ Pedidos de peÃ§as']] as const).map(([k,l]) => (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                    {['Produto', 'Unidade', 'Custo unit.', 'PreÃ§o venda', 'Margem', 'Status', 'AÃ§Ãµes'].map(h => (
                        ...
                        <td colSpan={7} style={{ padding: '8px 14px', background: '#fffbeb', borderBottom: '1px solid #fde68a', borderTop: '1px solid #fde68a' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 700, color: '#92400e', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                              Cadastros incompletos â€” {incompletos.length} produto{incompletos.length > 1 ? 's' : ''} sem preÃ§o ou custo
                            </span>
                          </div>
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td colSpan={7} style={{ padding: '10px 14px', background: '#f0fdf4', borderBottom: '1px solid #bbf7d0', borderTop: '2px solid #e2e8f0' }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: '#065f46', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                            âœ“ Produtos completos â€” {completos.length}
                          </span>
                        </td>
                      </tr>
                    </>
      ...
      {aba === 'entradas' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            <span style={{ alignSelf: 'center', color: '#94a3b8' }}>atÃ©</span>
          ...
          {loadingEntradas ? (
            ...
            <div style={{ textAlign: 'center', padding: 60 }}><div style={{ fontSize: 36, marginBottom: 12 }}>ðŸ“¥</div><p style={{ color: '#94a3b8' }}>Nenhuma entrada registrada</p></div>
          ) : (
                ...
                <tbody>
                  {entradasHistorico.map(e => (
                    <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '9px 14px', color: '#94a3b8', fontSize: 12 }}>{e.nota_fiscal ?? 'â€”'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {/* â”€â”€ ABA FORNECEDORES */}
      {aba === 'fornecedores' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          {loadingFornecedores ? (
            <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>Carregando...</div>
          ) : fornecedoresList.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 60 }}><div style={{ fontSize: 36, marginBottom: 12 }}>ðŸ­</div><p style={{ color: '#94a3b8' }}>Nenhum fornecedor cadastrado</p><button onClick={() => router.push('/fornecedores')} style={{ marginTop: 12, padding: '8px 16px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>Gerenciar fornecedores</button></div>
          ) : (
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ padding: '12px 18px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => router.push('/fornecedores')} style={{ fontSize: 12, color: '#2563eb', background: 'none', border: 'none', cursor: 'pointer' }}>Gerenciar completo â†’</button>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead><tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                ...
                <tbody>
                  {fornecedoresList.map(f => (
                    <tr key={f.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 12 }}>{f.telefone ?? 'â€”'}</td>
                      <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 12 }}>{f.email ?? 'â€”'}</td>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          {loadingPedidos ? (
            ...
            <div style={{ textAlign: 'center', padding: 60 }}>
              <div style={{ fontSize: 36, marginBottom: 12 }}>ðŸ”§</div>
              <p style={{ color: '#94a3b8', fontSize: 14 }}>Nenhuma peÃ§a pendente de chegada.</p>
            </div>
          ) : (
            <div>
              <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 16px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: '#92400e', fontWeight: 600 }}>â³ {pedidosPecas.length} peÃ§a(s) aguardando chegada</span>
              ...
              <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead><tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                    {['Aparelho','PeÃ§a','Valor estimado','Data pedido',''].map(h => <th key={h} style={{ padding: '9px 14px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {pedidosPecas.map(p => (
                      <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '10px 14px', fontWeight: 500, color: '#0f172a' }}>
                          {p.aparelhos ? `${p.aparelhos.marca} ${p.aparelhos.modelo}` : 'â€”'}
                        </td>
                        <td style={{ padding: '10px 14px', color: '#374151' }}>{p.peca_nome}</td>
                        <td style={{ padding: '10px 14px', fontFamily: 'monospace', color: '#374151' }}>
                          {p.preco > 0 ? `R$ ${p.preco.toFixed(2).replace('.',',')}` : 'â€”'}
                        </td>
      ...
      {/* â•â•â• MODAL ESCOLHA TIPO DE CADASTRO â•â•â• */}
      {showModalEscolha && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 440, padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              ...
              <button onClick={() => setShowModalEscolha(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#94a3b8', lineHeight: 1, marginTop: -4 }}>Ã—</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <button onClick={abrirNovoRapido} style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: '18px 20px', border: '2px solid #e2e8f0', borderRadius: 12, background: '#fff', cursor: 'pointer', textAlign: 'left', transition: 'border-color 0.15s' }}
                ...
                <span style={{ fontSize: 28, flexShrink: 0 }}>âš¡</span>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600, color: '#0f172a', marginBottom: 4 }}>Cadastro rÃ¡pido</p>
                  <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5 }}>Nome, cÃ³digo, preÃ§o e unidade. Ideal para cadastrar rapidamente durante uma venda ou OS.</p>
                </div>
              </button>
              <button onClick={abrirNovoCompleto} style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: '18px 20px', border: '2px solid #e2e8f0', borderRadius: 12, background: '#fff', cursor: 'pointer', textAlign: 'left', transition: 'border-color 0.15s' }}
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5 }}>Todas as informaÃ§Ãµes: fotos, fornecedor, categoria, dimensÃµes, dados fiscais (NCM, CSOSN, CFOP), preÃ§o atacado e mais.</p>
                </div>
              </button>
            </div>
          </div>
      ...
      {/* â•â•â• MODAL EDITAR RÃPIDO â•â•â• */}
      {showModalRapido && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 200, padding: '20px 16px', overflowY: 'auto' }}>
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 560, marginBottom: 20 }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', borderRadius: '16px 16px 0 0' }}>
              <div>
                <h2 style={{ fontSize: 16, fontWeight: 600, color: '#0f172a' }}>{fEditId ? 'âš¡ Editar rÃ¡pido' : 'ðŸ“¦ Novo produto'}</h2>
                <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>InformaÃ§Ãµes bÃ¡sicas Â· use "Editar completo" para todos os campos</p>
              </div>
              <button onClick={() => setShowModalRapido(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#94a3b8', lineHeight: 1 }}>Ã—</button>
            </div>
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={lbl}>CÃ³digo interno</label><input style={inp} value={fCodInterno} onChange={e => setFCodInterno(e.target.value)} placeholder="PRD-001" /></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                ...
                <div>
                  <label style={lbl}>PreÃ§o venda (R$) *</label>
              ...
              <div>
                <label style={lbl}>Modelos compatÃ­veis</label>
      ...
      {/* â•â•â• MODAL EDITAR COMPLETO â•â•â• */}
      {showModalCompleto && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 200, padding: '20px 16px', overflowY: 'auto' }}>
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 760, marginBottom: 20 }}>
            ...
            <div style={{ padding: '18px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', borderRadius: '16px 16px 0 0', position: 'sticky', top: 0, zIndex: 1 }}>
              <div>
                <h2 style={{ fontSize: 16, fontWeight: 600, color: '#0f172a' }}>ðŸ“‹ Cadastro completo</h2>
                <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{fNome || 'Novo produto'}</p>
              </div>
              <button onClick={() => setShowModalCompleto(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#94a3b8', lineHeight: 1 }}>Ã—</button>
            </div>

            {/* Tabs do modal */}
            <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid #e2e8f0', padding: '0 24px', background: '#fafafa' }}>
              {([
                ['ident', 'ðŸ“ IdentificaÃ§Ã£o'],
                ['valores', 'ðŸ’° Valores'],
                ['fiscal', 'ðŸ§¾ Fiscal'],
                ['dimensoes', 'ðŸ“ DimensÃµes'],
                ['fotos', `ðŸ–¼ Fotos${fcFotos.length > 0 ? ` (${fcFotos.length})` : ''}`],
              ] as const).map(([k, l]) => (
            ...
            <div style={{ padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {fcTab === 'ident' && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <div><label style={lbl}>CÃ³digo interno</label><input style={inp} value={fCodInterno} onChange={e => setFCodInterno(e.target.value)} placeholder="PRD-001" /></div>
                    <div><label style={lbl}>CÃ³digo de barras</label><input style={inp} value={fCodBarras} onChange={e => setFCodBarras(e.target.value)} /></div>
                    <div><label style={lbl}>Unidade</label>
                      <select style={inp} value={fUnidade} onChange={e => setFUnidade(e.target.value)}>
                        {['un','kg','g','m','cm','l','ml','cx','pc','par'].map(u => <option key={u} value={u}>{u}</option>)}
                      </select>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    ...
                    <div>
                      ...
                      <input style={inp} value={fcCategoria} onChange={e => setFcCategoria(e.target.value)} placeholder="Ex: Display, Bateria, CarcaÃ§a..." />
                    </div>
                  </div>
                  <div>
                    <label style={lbl}>Modelos compatÃ­veis (separados por vÃ­rgula)</label>
                  ...
                  <div>
                    ...
                    <textarea style={{ ...inp, minHeight: 72, resize: 'vertical' }} value={fDescricao} onChange={e => setFDescricao(e.target.value)} placeholder="Detalhes tÃ©cnicos, caracterÃ­sticas..." />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      <label style={lbl}>Estoque mÃ­nimo (alerta)</label>
              ...
              {fcTab === 'valores' && (
                <>
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>PreÃ§o de custo</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                      <div><label style={lbl}>Custo unitÃ¡rio (R$)</label>
                        <input style={inp} type="number" step="0.01" min="0" value={fCusto} onChange={e => {
                          setFCusto(e.target.value)
                          const c = parseFloat(e.target.value) || 0; const m = parseFloat(fMargem) || 0
                          if (m > 0 && c > 0) setFPrecoVenda(String(calcPreco(c, m)))
                      ...
                      <div><label style={lbl}>Despesas acessÃ³rias (R$)</label><input style={inp} type="number" step="0.01" min="0" value={fcDespAcess} onChange={e => setFcDespAcess(e.target.value)} /></div>
                    </div>
                  ...
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>PreÃ§o de venda</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 10 }}>
                      ...
                      <div><label style={lbl}>PreÃ§o de venda (R$) *</label>
                        <input style={inpReq(parseFloat(fPrecoVenda))} type="number" step="0.01" min="0" value={fPrecoVenda} onChange={e => {
                          setFPrecoVenda(e.target.value)
                          const c = parseFloat(fCusto) || 0; const p = parseFloat(e.target.value) || 0
                          if (c > 0 && p > c) setFMargem(String(Math.round((1 - c / p) * 10000) / 100))
                    ...
                    {parseFloat(fCusto) > 0 && parseFloat(fPrecoVenda) > 0 && (
                      <div style={{ background: '#dbeafe', border: '1px solid #bfdbfe', borderRadius: 8, padding: '10px 14px', display: 'flex', gap: 20, fontSize: 12 }}>
                        ...
                        <div><span style={{ color: '#64748b' }}>Markup: </span><strong style={{ color: '#1d4ed8' }}>{parseFloat(fCusto) > 0 ? ((parseFloat(fPrecoVenda) / parseFloat(fCusto) * 100) - 100).toFixed(1) : 'â€”'}%</strong></div>
                      </div>
                    )}
                  </div>
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>PreÃ§o atacado</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div><label style={lbl}>PreÃ§o atacado (R$)</label><input style={inp} type="number" step="0.01" min="0" value={fcPrecoAtacado} onChange={e => setFcPrecoAtacado(e.target.value)} placeholder="Opcional" /></div>
                      <div><label style={lbl}>Qtd. mÃ­nima atacado</label><input style={inp} type="number" min="1" value={fcQtdMinAtacado} onChange={e => setFcQtdMinAtacado(e.target.value)} placeholder="Ex: 5" /></div>
                    </div>
                  </div>
                </>
              )}
              ...
              {fcTab === 'fiscal' && (
                <>
                  <div style={{ background: fcHabNf ? '#f0fdf4' : '#f8fafc', border: `1px solid ${fcHabNf ? '#bbf7d0' : '#e2e8f0'}`, borderRadius: 8, padding: '12px 16px', marginBottom: 4 }}>
                    <Toggle value={fcHabNf} onChange={setFcHabNf} label="Habilitar emissÃ£o de NF-e para este produto" />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div><label style={lbl}>Origem</label>
                      <select style={inp} value={fcOrigem} onChange={e => setFcOrigem(e.target.value)}>
                        <option value="0">0 â€“ Nacional</option>
                        <option value="1">1 â€“ Estrangeira (importaÃ§Ã£o direta)</option>
                        <option value="2">2 â€“ Estrangeira (adquirida no mercado interno)</option>
                        <option value="3">3 â€“ Nacional, conteÃºdo importaÃ§Ã£o {'>'} 40%</option>
                        <option value="4">4 â€“ Nacional, processos produtivos bÃ¡sicos</option>
                        <option value="5">5 â€“ Nacional, conteÃºdo importaÃ§Ã£o {'<='} 40%</option>
                        <option value="6">6 â€“ Estrangeira, importaÃ§Ã£o direta, sem similar nacional</option>
                        <option value="7">7 â€“ Estrangeira, mercado interno, sem similar nacional</option>
                        <option value="8">8 â€“ Nacional, conteÃºdo importaÃ§Ã£o {'>'} 70%</option>
                      </select>
                    </div>
              ...
              {fcTab === 'dimensoes' && (
                <>
                  <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4 }}>Somente nÃºmeros. Usado para cÃ¡lculo de frete e NF-e.</p>
                  <div>
                  ...
                  {(fcPesoG || fcAltura || fcLargura || fcComprimento) && (
                    <div style={{ background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 8, padding: '12px 16px', fontSize: 12, color: '#0369a1' }}>
                      ðŸ“¦ Caixa: {fcAltura || '?'} Ã— {fcLargura || '?'} Ã— {fcComprimento || '?'} cm Â· Peso: {fcPesoG || '?'} g
                    </div>
                  )}
                </>
              )}
                ...
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>{fcFotos.length}/4 fotos Â· Formatos: JPG, PNG, WebP Â· MÃ¡x. 5 MB por foto</p>
                    </div>
                    {fcFotos.length < 4 && (
                      ...
                      {fcFotos.map((foto, i) => (
                        <div key={i} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid #e2e8f0', aspectRatio: '4/3', background: '#f8fafc' }}>
                          <img src={foto.url} alt={`Foto ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
                            onError={e => { (e.target as HTMLImageElement).src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y="50%" dominant-baseline="middle" text-anchor="middle" font-size="40">ðŸ–¼</text></svg>' }} />
                          {foto.file && (
                  ...
                  {fcFotos.some(f => f.file) && (
                    <div style={{ marginTop: 12, padding: '8px 12px', background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 8, fontSize: 12, color: '#92400e' }}>
                      âš  {fcFotos.filter(f => f.file).length} foto(s) pendente(s) de upload â€” serÃ£o enviadas ao salvar.
                    </div>
                  )}
                </>
              )}
            ...
            <div style={{ padding: '14px 24px', borderTop: '1px solid #e2e8f0', display: 'flex', gap: 8, justifyContent: 'space-between', background: '#f8fafc', borderRadius: '0 0 16px 16px' }}>
              <div style={{ fontSize: 12, color: '#94a3b8', alignSelf: 'center' }}>
                {(['ident','valores','fiscal','dimensoes','fotos'] as const).map((t, i) => (
                  <span key={t} onClick={() => setFcTab(t)} style={{ cursor: 'pointer', marginRight: 4, fontSize: 14, color: fcTab === t ? '#2563eb' : '#d1d5db' }}>â—</span>
                ))}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={salvarCompleto} disabled={fSaving || !fNome.trim()}
                  style={{ padding: '9px 22px', background: fSaving || !fNome.trim() ? '#93c5fd' : '#2563eb', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: fSaving || !fNome.trim() ? 'not-allowed' : 'pointer' }}>
                  {fcUploadando ? 'ðŸ“¤ Enviando fotos...' : fSaving ? 'Salvando...' : 'âœ“ Salvar cadastro completo'}
                </button>
              </div>
            </div>
          </div>
      ...
      {/* â•â•â• MODAL ENTRADA DE CUSTO â•â•â• */}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ padding: '18px 22px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowModalEntrada(false)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#94a3b8' }}>Ã—</button>
            </div>
            <div style={{ padding: '18px 22px' }}>
              {(produtoSel.custo_unit ?? 0) > 0 && (
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '12px 16px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <p style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 2 }}>Custo unitÃ¡rio atual</p>
                  ...
                  {ultimoCusto && (
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 2 }}>Ãšltima compra</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                ...
                <div style={{ gridColumn: '1/-1' }}>
                  <label style={lbl}>Custo unitÃ¡rio pago (R$){ultimoCusto && <span style={{ color: '#94a3b8', fontWeight: 400, marginLeft: 6 }}>Ãºltimo: R$ {ultimoCusto.toFixed(2).replace('.', ',')}</span>}</label>
                  ...
                  {ultimoCusto && eCusto && parseFloat(eCusto) !== ultimoCusto && (
                    <p style={{ fontSize: 11, marginTop: 4, color: parseFloat(eCusto) > ultimoCusto ? '#991b1b' : '#065f46' }}>
                      {parseFloat(eCusto) > ultimoCusto ? `â–² ${((parseFloat(eCusto) - ultimoCusto) / ultimoCusto * 100).toFixed(1)}% mais caro` : `â–¼ ${((ultimoCusto - parseFloat(eCusto)) / ultimoCusto * 100).toFixed(1)}% mais barato`} que a Ãºltima compra
                    </p>
                  )}
                </div>
                <div style={{ gridColumn: '1/-1' }}>
                  <label style={lbl}>Nota fiscal / referÃªncia (opcional)</label>
                  <input style={inp} value={eNF} onChange={e => setENF(e.target.value)} placeholder="NF-e, nÃºmero do pedido..." />
                </div>
              </div>
                ...
                <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '12px 16px', marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 20 }}>
                    ...
                    <div><p style={{ fontSize: 11, color: '#64748b' }}>Margem estimada</p><p style={{ fontSize: 15, fontWeight: 600, color: produtoSel.preco_venda > parseFloat(eCusto) ? '#065f46' : '#991b1b' }}>
                      {produtoSel.preco_venda > 0 ? `${((produtoSel.preco_venda - parseFloat(eCusto)) / produtoSel.preco_venda * 100).toFixed(0)}%` : 'â€”'}
                    </p></div>
                  </div>
                </div>
              )}
              {entradas.length > 0 && (
                <div>
                  <p style={{ fontSize: 11, fontWeight: 500, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8 }}>HistÃ³rico de compras</p>
                    ...
                    {entradas.map((e, i) => (
                      <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 14px', borderBottom: i < entradas.length - 1 ? '1px solid #f1f5f9' : 'none', background: i === 0 ? '#f0f9ff' : '#fff' }}>
                        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                          {i === 0 && <span style={{ fontSize: 10, fontWeight: 500, background: '#dbeafe', color: '#1d4ed8', padding: '1px 6px', borderRadius: 10 }}>Ãºltima</span>}
